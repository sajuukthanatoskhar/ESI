...
class MyFirstGUI:
    def __init__(self, master):
        ...
        basicorenames = ['Atmospheric Gases', 'Cadmium', 'Caesium', 'Chromium' ,'Cobalt' , 'Dysprosium' ,'Evaporite Deposits' ,'Hafnium' , 'Hydrocarbons', 'Mercury', 'Neodymium', 'Platinum', 'Promethium', 'Scandium', 'Silicates', 'Technetium', 'Thulium', 'Titanium', 'Tungsten', 'Vanadium']
        advancedreactions = ['Caesarium Cadmide','Carbon Polymers','Ceramic Powder','Crystallite Alloy','Dysporite','Fernite Alloy','Ferrofluid','Fluxed Condensates','Hexite','Hyperflurite','Neo Mercurite'
        ...
        compositereactions = ['Crystalline Carbonide','Fermionic Condensates','Fernite Carbide','Ferrogel','Fullerides','Hypersynaptic Fibers','Nanotransistors','Nonlinear Metamaterials',                              'Phenolic Composites','Photonic Metamaterials','Plasmonic Metamaterials','Sylramic Fibers','Terahertz Metamaterials','Titanium Carbide','Tungsten Carbide']
        BasicOreWantToUse = []
        BasicOreLabels = []
        BasicOreInventory = []
        ...
        CompositeLabels = []
        CompositeInventory = []
        CompositeIncrements = [10000,400,10000,400,3000,750,1500,300,2200,300,300,6000,300,10000,10000]
        ...
        for k in range(0,len(compositereactions)):
            ...
            CompositeWantToUse.append(Spinbox(master, from_=0, increment=CompositeIncrements[k], to=maximum*CompositeIncrements[k]))
        ...
        self.update_button = Button(master, text="Update", command=lambda: self.update(BasicOreInventory,BasicOreWantToUse)).grid(row=23, column=1)
        self.update_button = Button(master, text="Update Basic Reactions",command=lambda: self.update(BasicOreInventory, BasicOreWantToUse,AdvancedReactionsInventory,AdvancedReactionsWantToUse,CompositeIncrements,CompositeWantToUse)).grid(row=23,column=4)
        self.update_button = Button(master, text="Update Basic Reactions",command=lambda: self.updatebasic(BasicOreInventory, BasicOreWantToUse, AdvancedReactionsInventory, AdvancedReactionsWantToUse,CompositeIncrements,CompositeWantToUse)).grid(row=23, column=4)
        self.update_button = Button(master, text="Update Complex", command=lambda: self.updatecomplexupwards(AdvancedReactionsInventory,CompositeInventory,complexmatsreqs,CompositeIncrements)).grid(row=23,column=7)
        ...
        self.update_button = Button(master, text="Print to Console", command=lambda: self.printvalue(BasicOreLabels,BasicOreWantToUse)).grid(row=23,column=6)
    ...
    def printvalue(self,BasicOreLabels,BasicOreWantToUse):
        ...
        for i in range(0,len(BasicOreWantToUse)):
            if int(BasicOreWantToUse[i].get()) > 0:
                ...
                totalcost = market_and_fleet.get_market_price("The Forge",BasicOreLabels[i]["text"])*int(BasicOreWantToUse[i].get()) + totalcost
    ...
    def updatebasic(self,BasicOreInventory,BasicOreWantToUse,AdvancedReactionsInventory, AdvancedReactionsWantToUse, CompIncrement,CompositeWant):
